...
namespace StereoKit.Framework
{
	public static class HolographicTheme
	{
		...
		static Mesh GlassButtonMesh(float cornerRadius, int cornerResolution)
		{
			...
			// How much smaller the inner quad is from the edge, as
			// compared to the overall footprint of the button.
			float innerquadInset = 0.001f;

			float frontplateZ = 1.0f;
			float backplateZ = 0.1f;
			...
			for (int c = 0; c < 4; c++)
			{
				...
				uint vert = (uint)(c * (usubd + 2));
				verts[vert] = new Vertex(offset + V.XYZ(0, 0, backplateZ), V.XYZ(0, 0, -1), V.XY(u, v), new Color32(255, 0, 255, 255));
				verts[vert + 1] = new Vertex(offsetFront - V.XYZ(0, 0, frontplateZ), V.XYZ(0, 0, -1), V.XY(u, v), new Color32(255, 255, 0, 255));
				...
				for (uint i = 0; i < cornerResolution; i++)
				{
					...
					Vec3 backPos = normal * (radius - innerquadInset) + V.XYZ(0, 0, backplateZ);
					Vec3 FrontPos = normal * radius - V.XYZ(0, 0, frontplateZ);
